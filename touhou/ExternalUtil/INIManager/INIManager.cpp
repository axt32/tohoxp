...
{
	BOOL bResult = WritePrivateProfileStringA(Group.c_str(), Category.c_str(), IN_value.c_str(), sPath.c_str());
	if (bResult == TRUE)
	{
		return true;
	}
...
{
	char sResult[255] = "";
	GetPrivateProfileStringA(Group.c_str(), Category.c_str(), "failed", sResult, 255, sPath.c_str());
